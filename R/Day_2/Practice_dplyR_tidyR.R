...
# -------------------------------------------------------------------------
# Motivação: resumir várias colunas de várias formas diferentes.
...
# Duas modificações em um mesmo across()
casas %>%
...
# Duas modificações em um mesmo across() + renomeação
casas %>%
  summarise(across(
    .fns = list(num_nas = ~sum(is.na(.x)), num_cats = n_distinct),
    .names = "{.fn}_{.col}" # Fórmula antiga: "{.col}_{.fn}"
  )) %>%
  glimpse()
